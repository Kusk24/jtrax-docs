...
| nginx | 80 → web, 8080 → admin | systemd | ~10 MB |
...
**The admin console is on port 8080, not a subpath.** Neither Next app sets
...
**nginx does not proxy the Go backend at all.** Both Next apps reach it server-side
...
**Firewall:** 8080 open but restricted to a single source address, because the admin
login is still plain HTTP; 443 open to all, ahead of TLS. A home ISP address is
dynamic, so that 8080 rule will go stale and present as "the server is down".
